...
const Sidebar = ({ collapsed, setCollapsed }) => {
  ...
  return (
    <aside
      className={`fixed top-0 left-0 h-full z-40 bg-[var(--surface)] border-r border-[var(--border)] flex flex-col transition-all duration-300 ${collapsed ? "w-[72px]" : "w-60"}`}
    >
      {/* Logo */}
      <div
        className={`flex items-center h-16 px-4 border-b border-[var(--border)] ${collapsed ? "justify-center" : "gap-3"}`}
      >
      ...
      <nav className="flex-1 py-4 px-3 space-y-1 overflow-y-auto">
        {navItems.map(({ to, icon: Icon, label }) => (
          ...
          >
            <Icon
              size={20}
              className={`flex-shrink-0 transition-colors ${isActive(to) ? "text-[var(--accent)]" : "text-[var(--text-muted)] group-hover:text-[var(--text-secondary)]"}`}
            />
