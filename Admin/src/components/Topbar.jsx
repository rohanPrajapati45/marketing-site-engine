...
const Topbar = ({ onMenuClick }) => {
  ...
  const getTitle = () => {
    if (location.pathname.startsWith('/pages/') && location.pathname !== '/pages') {
      return 'Page Builder';
    }
    if (location.pathname.startsWith('/blogs/')) {
      return location.pathname.includes('/new') ? 'New Blog' : 'Edit Blog';
    }
  ...
  return (
    <header className="h-16 bg-[var(--surface)]/80 backdrop-blur-xl border-b border-[var(--border)] flex items-center justify-between px-6 sticky top-0 z-30">
      <div className="flex items-center gap-3">
        <button onClick={onMenuClick} className="lg:hidden p-2 rounded-lg hover:bg-[var(--hover)] text-[var(--text-muted)]">
          <Menu size={20} />
        </button>
        <h1 className="text-lg font-semibold text-[var(--text-primary)]">{getTitle()}</h1>
      </div>

      <div className="flex items-center gap-3">
        <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-lg bg-[var(--hover)]">
          <User size={14} className="text-[var(--text-muted)]" />
          <span className="text-xs text-[var(--text-secondary)] font-medium">{admin?.email}</span>
        </div>
        <button
